...
#define NUTS_FAIL(cond, expect)                                                \
    do {                                                                       \
        ...
        TEST_MSG("%s: expected %s (%d), got %s (%d)", #cond,                   \
            nng_strerror(expect), expect, nng_strerror(result_), result_);     \
    } while (0)
...
#define NUTS_RECV(sock, string)                                                \
    do {                                                                       \
        ...
        TEST_CHECK_(rv_ == 0, "nng_recv (%d %s)", rv_, nng_strerror(rv_));     \
        TEST_CHECK_(sz_ == strlen(string) + 1, "length %d want %d", (int)sz_,  \
            (int)strlen(string) + 1);                                          \
        buf_[sizeof(buf_) - 1] = '\0';                                         \
        TEST_CHECK_(strcmp(string, buf_) == 0, "%s == %s", string, buf_);      \
    } while (0)

#define NUTS_MATCH(s1, s2)                                                     \
    do {                                                                       \
        TEST_CHECK_(strcmp(s1, s2) == 0, "%s == %s", (char*)s1, (char*)s2);    \
    } while (0)
